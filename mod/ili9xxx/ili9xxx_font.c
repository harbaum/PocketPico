...
// ili9xxx_blit does not support the ili9341 yet, nor does it
// support the 1.5x scaling
#ifndef ILI9341

void ili9xxx_blit(uint16_t *fbuf, uint8_t x, uint8_t y, uint8_t w, uint8_t h) {
    ili9xxx_set_window(x, w, y, h);
    ili9225_write_cmd(ILI9225_REG_GRAM_RW);
    ili9xxx_set_rc_cs(1, 0);

    for(uint8_t i = 0; i < (w * h); i++) {
        ili9xxx_write_pixel(fbuf[i]);
    }

    ili9xxx_set_rc_cs(1, 1);
}

void ili9xxx_get_letter(uint16_t *fbuf, char l,uint16_t color,uint16_t bgcolor) {
    uint8_t letter[8];
    uint8_t row;
...
void ili9xxx_text(char *s, uint8_t x, uint8_t y, uint16_t color, uint16_t bgcolor) {
    uint16_t fbuf[8 * 8];
    for(uint8_t i = 0; i < strlen(s); i++) {
        ili9xxx_get_letter(fbuf, s[i], color, bgcolor);
        ili9xxx_blit(fbuf, x, y, 8, 8);
        x += 8;
        if(x > 27 * 8) {
            break;
        }
